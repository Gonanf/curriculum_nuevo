...
function ParallaxSection({ children, className = '', backgroundY = ["-30%", "-15%", "0%", "10%", "30%"], contentY = ["-30%", "0", "30vh"], backgroundOP = [0, 1, 0], contentOP = [0, 1, 1, 0], contentClass = 'z-10' }: Props) {
  const ref = useRef(null)

  const { scrollYProgress } = useScroll({ target: ref, offset: ["start end", "end start"] })

  const bgY = useTransform(scrollYProgress, [0, 0.25, 0.5, 0.75, 1], backgroundY)
  ...
  return (<motion.section className={className + " relative min-h-[150%]"} style={{ opacity: bgOpacity, y: bgY }}>
    <motion.div ref={ref} className={contentClass + " sticky inset-0"} style={{ opacity: contentOpacity }}>
      {children}
    </motion.div>
  </motion.section>)
}
